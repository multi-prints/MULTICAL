...
async fn tauri_invoke_inner<T: for<'de> Deserialize<'de>>(cmd: &str, args: &impl Serialize) -> Result<T, String> {
    ...
    let f: js_sys::Function = fn_val.dyn_into()
        .map_err(|_| "window.tauriInvoke is not a function".to_string())?;
    
    // Call the function — returns a Promise
    let prom = f.call2(&JsValue::UNDEFINED, &JsValue::from_str(cmd), &JsValue::from_str(&args_json))
        .map_err(|e| format!("tauriInvoke call failed: {:?}", e))?;
    let promise: js_sys::Promise = prom.dyn_into()
        .map_err(|_| "tauriInvoke did not return a Promise".to_string())?;
    
    // Await the Promise
    let result = JsFuture::from(promise).await
        .map_err(|e| e.as_string().unwrap_or_else(|| format!("IPC rejected for '{}'", cmd)))?;
    
    let raw_str = result.as_string().ok_or_else(|| format!("IPC response not a string for '{}'", cmd))?;
    
    // Parse the wrapper: { ok: ... } or { err: ... }
    let envelope: serde_json::Value = serde_json::from_str(&raw_str).map_err(|e| {
        format!("Failed to parse IPC response for '{}': {} - raw: {}", cmd, e, &raw_str[..raw_str.len().min(200)])
    })?;
    if let Some(err) = envelope.get("err").and_then(|v| v.as_str()) {
        return Err(err.to_string());
    }
    let ok_val = envelope.get("ok").ok_or_else(|| {
        format!("No 'ok' field in IPC response for '{}': {}", cmd, raw_str)
    })?;
    serde_json::from_value(ok_val.clone()).map_err(|e| {
        format!("Deserialize error for '{}': {} - raw: {}", cmd, e, &raw_str[..raw_str.len().min(200)])
    })
}
...
fn default_size() -> String { "1".into() }
fn default_sticker_type() -> String { "colored".into() }
...
fn default_payment() -> String { "cash".into() }
fn default_customer() -> String { "Walk-in".into() }
...
api_fn!(add_stock, "add_stock", stockItem: &NewStockItem, StockItem);
...
api_fn!(get_debt_payments, "get_debt_payments", debtId: i64, Vec<DebtPayment>);
...
pub async fn update_printing_material(id: i64, updates: &serde_json::Value) -> Result<SuccessResponse, String> {
    tauri_invoke_inner("update_printing_material", &serde_json::json!({ "id": id, "updates": updates })).await
}
